...
class scDataset(Dataset):
    ...
    def __init__(self, csr, classes, scaler, n_mask, batch_size, max_value=10, inference=False, pin_memory=False):
        self.csr = csr
    ...
    def __getitem__(self, idx: list):
        ...
        # for training
            ...
            # mask indices
            mask_col_ids = torch.tensor(np.concatenate([np.random.choice(self.csr[i].indices, self.n_mask, replace=False) for i in idx])).long()
            mask_row_ids = torch.repeat_interleave(torch.arange(0, self.batch_size), self.n_mask).long()
            ...
            mask_vals = gene_vals[mask_row_ids, mask_col_ids].reshape(self.batch_size, -1)
...
class ExceiverDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument("--data_path", type=Path, help="Path to directory with preprocessed data.")
        parser.add_argument("--classify", type=str, default=None, help="Name of column from `obs` table to add classification task with. (optional)")
        parser.add_argument("--frac", type=float, default=0.15, help="Fraction of median genes to mask for prediction.")
    ...
    def prepare_data(self):
        ...
        self.n_mask = int(self.train_adata.obs['n_genes_by_counts'].median() * self.frac)
        ...
        if self.classify:
            if not np.array_equal(np.unique(self.train_adata.obs[self.classify]), np.unique(self.val_adata.obs[self.classify])):
            ...
            self.train_classes = np.array([self.classify_dict[code] for code in self.train_adata.obs[self.classify]])
            self.val_classes = np.array([self.classify_dict[code] for code in self.val_adata.obs[self.classify]])
    ...
    def setup(self, stage):
        ...
        if stage == 'fit':
            ...
            self.train_dataset = scDataset(self.train_adata.X, classes=self.train_classes, scaler=self.train_scaler, n_mask=self.n_mask, batch_size=self.batch_size, pin_memory=False)
            self.val_dataset = scDataset(self.val_adata.X, classes=self.val_classes, scaler=self.val_scaler, n_mask=self.n_mask, batch_size=self.batch_size, pin_memory=False)
    ...
    def train_dataloader(self, num_workers=6):
        sampler = BatchSampler(RandomSampler(self.train_dataset), batch_size=self.train_dataset.batch_size, drop_last=True)
        dl = DataLoader(self.train_dataset, batch_size=None, batch_sampler=None, sampler=sampler, num_workers=num_workers, pin_memory=True)
        return dl

    def val_dataloader(self, num_workers=6):
        sampler = BatchSampler(SequentialSampler(self.val_dataset), batch_size=self.val_dataset.batch_size, drop_last=True)
        dl = DataLoader(self.val_dataset, batch_size=None, batch_sampler=None, sampler=sampler, num_workers=num_workers, pin_memory=True)
        return dl
