...
class CrossAttn(nn.Module):
    ...
    def __init__(self, query_dim, key_val_dim, num_heads=4, dropout=0.0):
        ...
        self.cross_attn = nn.MultiheadAttention(query_dim, 
                                                num_heads, 
                                                dropout=dropout, 
                                                kdim=key_val_dim, 
                                                vdim=key_val_dim, 
                                                batch_first=True)
        self.mlp = nn.Sequential(nn.LayerNorm(query_dim), 
...
class ProcessSelfAttn(nn.Module):
    ...
    def __init__(self, embed_dim, num_layers, nhead, dim_feedforward=2048, dropout=0.2):
        super().__init__()
        self.encoder_layer = nn.TransformerEncoderLayer(embed_dim, nhead, dim_feedforward, dropout, activation='gelu', batch_first=True)
...
class Exceiver(pl.LightningModule):
    ...
    def __init__(self, seq_len, seq_dim, query_len, query_dim, 
                 num_layers: int=1, nhead: int=1, dim_feedforward: int=2048, dropout: float=0.0, # process attn module
                 learning_rate: float=3e-4, weight_decay: float=1e-5, warmup_steps: int=500, **kwargs):
        ...
        self.process_self_attn = ProcessSelfAttn(query_dim, num_layers, nhead, dim_feedforward, dropout)
        self.decoder_cross_attn = CrossAttn(seq_dim, query_dim) # query is now gene embedding

        # MLP for gene expression prediction
        self.mlp = nn.Sequential(nn.LayerNorm(seq_dim),
                                 nn.Linear(seq_dim, 1),
                                 nn.GELU(),
                                 nn.Linear(1, 1))
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument('--seq_dim', type=int, default=4, help="Dimension of gene representations.")
        parser.add_argument('--query_len', type=int, default=64, help="Size of input query, or latent representation length.")
        parser.add_argument('--query_dim', type=int, default=4, help="Dimension of input query.")
        parser.add_argument('--num_layers', type=int, default=1, help="Number of ProcessSelfAttention layers.")
        parser.add_argument('--nhead', type=int, default=1, help="Number of ProcessSelfAttention heads.")
        parser.add_argument('--dim_feedforward', type=int, default=2048, help="Dimension of ProcessSelfAttention feedforward network.")
        parser.add_argument('--dropout', type=float, default=0.0, help="Value of ProcessSelfAttention dropout.")
        parser.add_argument('--learning_rate', type=float, default=3e-4, help="Learning rate.")
        parser.add_argument('--weight_decay', type=float, default=1e-5, help="Optimizer weight decay.")
        parser.add_argument('--warmup_steps', type=int, default=500, help="Number of learning rate warmup steps (linear).")
        return parser

    def encoder_attn_step(self, gene_ids, gene_vals, input_query, key_padding_mask):
        gene_emb = self.gene_emb(gene_ids.long())
        # TODO: are there other / better ways of combining this information into the key?
        key_val = gene_vals.unsqueeze(2) * gene_emb
        latent, encoder_weights = self.encoder_cross_attn(input_query, key_val, key_padding_mask)
        return latent, encoder_weights

    def forward(self, gene_ids, gene_vals, input_query, key_padding_mask, mask_ids):
        # TODO: consider backprop conflicts between output_query and inputs on different samples
        output_query = self.gene_emb(mask_ids)
        latent, encoder_weights = self.encoder_attn_step(gene_ids, gene_vals, input_query, key_padding_mask)
        latent = self.process_self_attn(latent)
        # TODO: does adding output_query key_padding_mask constraint help model converge to sample identity?
        decoder_out, decoder_weights = self.decoder_cross_attn(output_query, latent, key_padding_mask=None)
        return decoder_out

    def training_step(self, batch, batch_idx):
        gene_ids, gene_vals, mask_ids, mask_vals, key_padding_mask = batch
        input_query = self.query_emb.repeat(len(gene_ids), 1, 1)
        decoder_out = self.forward(gene_ids, gene_vals, input_query, key_padding_mask, mask_ids)
    ...
    def optimizer_step(self, current_epoch, batch_nb, optimizer, optimizer_idx, closure, on_tpu=False, using_native_amp=False, using_lbfgs=False):
        ...
        if self.trainer.global_step < self.hparams.warmup_steps:
            lr_scale = min(1., float(self.trainer.global_step + 1) / float(self.hparams.warmup_steps))
...
class ExceiverClassifier(Exceiver):
    ...
    def __init__(self, seq_len, seq_dim, query_len, query_dim, classify_dim=None,
                 num_layers: int=1, nhead: int=1, dim_feedforward: int=2048, dropout: float=0.0, # process attn module
                 learning_rate: float=3e-4, weight_decay: float=1e-5, warmup_steps: int=500, **kwargs):

        # Initialize superclass
        super().__init__(
            seq_len, seq_dim, query_len, query_dim,
        ...
        self.mlp_classify = nn.Sequential(nn.LayerNorm(seq_dim),
    ...
    def training_step(self, batch, batch_idx):
        ...
        classify_hat = torch.squeeze(self.mlp_classify(decoder_out[:, mask_ids.shape[1], :]))
        loss = self.mse(y_hat, mask_vals.unsqueeze(2))
        ce_loss = self.cel(classify_hat, classify)
        class_metrics = self.train_class_metrics(torch.argmax(classify_hat, dim = 1), classify)
        metrics = self.train_metrics(y_hat, mask_vals.unsqueeze(2))
        loss_dict = {'train_MSELoss': loss, 'train_CELoss': ce_loss}
        self.log_dict(loss_dict | metrics | class_metrics, on_step=False, on_epoch=True, sync_dist=True)
        return loss + ce_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.mse(y_hat, mask_vals.unsqueeze(2))
        ce_loss = self.cel(classify_hat, classify)
        class_metrics = self.val_class_metrics(torch.argmax(classify_hat, dim = 1), classify)
        metrics = self.val_metrics(y_hat, mask_vals.unsqueeze(2))
    ...
    def test_step_end(self, results):
        ...
        loss = self.mse(y_hat, mask_vals.unsqueeze(2))
        ce_loss = self.cel(classify_hat, classify)
        class_metrics = self.test_class_metrics(torch.argmax(classify_hat, dim = 1), classify)
        metrics = self.test_metrics(y_hat, mask_vals.unsqueeze(2))
